...
from input_modules.base_class import PageIterator, SARFormat
        ...
        while header := self.file.read(8):
            if header[:2] == SYNC:
                ...
                if block_id == 4023:
                    ...
                    if crc_passed and band == 'GAL_L1BC' and tow != 'DNU' and wn_c != 'DNU':
                        tow = tow // 1000 - 2
                        nav_bits = BitArray(hex="".join(nav_bits_hex))[:234]
                        nav_bits.insert('0b000000', 114)
                        sar_format = SARFormat(svid, wn_c, tow, nav_bits)
                        self.file_pos = self.file.tell()
                        break
